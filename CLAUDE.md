...
- マージは**PR経由・merge commit（`--no-ff`）固定**（`git log --graph`で枝が見えるように。
  squash/rebaseは使わない）。PR作成時、Claudeが「変更概要／実装した機能・修正内容／
  レビュー観点／動作確認／関連リンク」をテンプレートに沿って記入する。
- **PR作成〜マージまでClaudeが実行してよい。** 「PR出して」＝作成まで、「マージまでやって」
  ＝`gh pr merge <番号> --merge --delete-branch`まで（CIのgreenは待たない）。
