...
# hqのGit運用ルールを導入（ブランチ＋PR＋merge commit、開発スピード優先）
...
- 以降、`main`への直pushはせず、変更はブランチ＋PR（merge commit（--no-ff）固定）
- **PR作成〜マージまで、依頼されたセッションのClaudeが実行してよい**（CIのgreenも
  待たない）。相手のApproveは不要
...
現段階では開発スピードを優先する判断。プロセスは「ブランチ＋PR＋merge commitの記録」だけ
