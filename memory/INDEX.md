...
- [2026-09-07 見つける画面の遷移ごとの再レンダリング遅延を削減](20260907_discover-render-latency.md) — フィルタが全てフルのサーバー遷移で毎回 Clerk API 1往復＋DB直列4往復。`loading.tsx` 追加／`currentUser()`→`auth()`／独立クエリを `Promise.all` 並列化。インデックス追加はデータ増加時の後回し
