...
- [2026-09-07 本番の一過性エラーとエラーバウンダリ整備（Neon コールドスタート）](20260907_prod-error-boundary-and-neon-cold-start.md) — 本番でERROR 2379659138＝Neon scale-to-zeroのコールドスタート失敗＋`error.tsx`不在。`(app)/error.tsx`・`global-error.tsx` 追加＋見つける画面のDB読み取りに1回リトライ。本番DBは trip 0件（シード別タスク）
