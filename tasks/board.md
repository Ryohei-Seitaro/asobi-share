...
- [ ] [frontend] ①修正：カレンダー登録でエラー（`oauth_token_retrieval_error`）→ Clerkの例外を握って「.icsから取り込んで」を表示し500を防ぐ。`AddToCalendar`は.icsを主ボタンに、Google直接登録は「準備中」表記に。保存ボタンでのカレンダーポップアップ自動表示を廃止（カレンダーボタンは残す）。Google直接登録の本対応はtasks未完了[infra][backend]項目 (起票: Seitaro, 2026-09-06)
