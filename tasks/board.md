...
- [ ] [frontend][backend] 購入済みの有料記事に「購入済み」マークを付ける。見つけるのカードに `PurchasedBadge`（右上、moneyカラー）＋価格チップを「購入済み」表示に。`page.tsx` で `tripPurchases` から `purchasedTripIds` を集計。旅程詳細ヘッダーへの表示は #12(⑦) 側に寄せる予定 (起票: Seitaro, 2026-09-06)
