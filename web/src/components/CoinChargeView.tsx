...
export function CoinChargeView({ initialBalance }: { initialBalance: number }) {
  const [amountYen, setAmountYen] = useState(1000);
  const [customYen, setCustomYen] = useState("");
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await chargeCoin(effectiveYen);
      if (result.ok) {
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 pt-[18px]">
        ...
        {done && !error && <p className="mb-3 text-[12px] leading-[1.5] text-coin">チャージが完了しました。</p>}
